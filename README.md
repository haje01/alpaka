...
알파카는 Kubernetes 클러스터에 Kafka + 관련 패키지를 배포하기 위한 Helm 차트이다. 다음과 같은 패키지를 포함한다:
...
여기에는 vim, kcat, jq, 파이썬 및 DNS 유틸 등이 설치되어 있기에 클러스터 내 다른 컨테이너의 동작을 확인하거나 네트워크 이슈를 찾는 등 다양한 작업을 할 수 있다. 기본 툴 컨테이너 이미지는 [도커 허브](https://hub.docker.com/) 에 `haje01/alpaka-tool` 으로 올라가 있기에 이것을 사용하면 된다. 

> 만약 독자적인 툴 이미지가 필요하다면 있으면 `images/Dockerfile.tool` 및 `images/build_tool.sh` 파일을 참고하여 만들면 될 것이다. 
...
기본 DB 커넥트 이미지는 [도커 허브](https://hub.docker.com/) 에 `haje01/kafka-dbcon` 으로 올라가 있기에 이것을 사용하면 된다. 

> 만약 독자적인 DB 커넥터 이미지가 필요하다면 있으면 `images/Dockerfile.dbcon` 및 `images/build_dbcon.sh` 파일을 참고하여 만들면 될 것이다. 
...
카프카는 [bitnami 의 kafka Helm 차트](https://bitnami.com/stack/kafka/helm) 를 이용한다. 차트의 기본값을 확인하기 위해서 다음과 같이 명령한다.
...
커넥터는 데이터를 카프카로 가져오거나 외부로 내보내기 위한 일종의 플러그인이다. 커넥트는 커넥터를 기동하기 위한 장비로 생각하면 간단하다.
...
최종 결과물은 각 커넥터 별로 등록 가능한 쉘 스크립트 형식으로 저장되는데, 그것은 `[커넥트 타입]-[커넥터 그룹]-[커넥터 이름].sh` 형식 이름의 실행가능한 파일로 뒤에서 설명할 **초기화 컨테이너** 에 저장된다. 

예제 설정의 경우 `/usr/local/bin` 디렉토리 아래 다음과 같은 세가지 파일이 생성되게 된다.
...
UI for Kafka 는 카프카 클러스터의 관리 및 모니터링에 사용되는데, 이를 위해 연결할 카프카 브로커, 주키퍼 그리고 필요한 경우 커넥트 정보를 기술해주어야 한다. 일반적으로 다음과 같은 구성이다.
