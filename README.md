...
만약 설정 파일내에 DB 암호 같은 보안상 중요한 내용이 있다면 설정에 기입한체 형상 관리툴에 등록하기는 꺼림직하다. Helm 이 변수 파일 내에서 환경변수를 참조할 수 있으면 좋겠으나 그런 기능은 지원하지 않고 있다. 이에 리눅스 쉘 명려어인 `envsubst` 를 이용하는 다음과 같은 방법을 추천한다.

1. 설정 파일내에 민감 정보 필드 값을 `${환경변수이름}` 처럼 표시한다.
  예) `db_pass: ${MY_DB_PASS}`
2. 설정 파일명에 `_` 를 붙여 그대로 사용될 수 없다는 것을 표시한다.
  예) `mymkb.yaml` -> `_mymkb.yaml`
3. `envsubst` 을 이용해 텍스트 파일 내 환경변수 참조를 값으로 치환하고 저장한다.
  예) `MY_DB_PASS=123456 envsubst < _mymkb.yaml > mymkb.yaml
4. 저장된 설정 파일을 이용해 설치한다.
  예) `helm install -f mymkb.yaml my alpaka/`
