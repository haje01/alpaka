...
두 방법 모두 이용하는 쿠버네티스 배포판에 맞는 설정이 필요한데, `configs/` 디렉토리에 아래와 같은 샘플 설정 파일이 있으니 참고하도록 하자 (샘플 설정 파일은 구분하기 쉽게 `_` 로 시작한다):
...
여기서는 설명을 위해 배포판 샘플 설정파일을 그대로 이용했지만, 실제로는 샘플 파일을 복사하여 업무 환경에 맞는 자신만의 설정 파일을 만들어 쓰는 것이 맞겠다.

> 여기서는 편의상 설정 파일명과 배포 이름을 같게 하였다. 실제로는 필요에 따라 배포 이름을 다르게 줄 수 있다.
